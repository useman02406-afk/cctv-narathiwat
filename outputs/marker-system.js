...
(() => {
  ...
  const icon = (kind, options = {}) => {
    ...
    const rawText = options.text || options.uid || '';
    const textLines = (Array.isArray(rawText) ? rawText : String(rawText).split('\n')).map(value => String(value || '').trim()).filter(Boolean).slice(0, 3);
    const hasText = textLines.length > 0;
    const labelHtml = textLines.map(text => `<small style="display:block;max-width:150px;padding:1px 5px;background:#fff;color:#172033;border:1px solid #cbd5e1;font:700 10px/1.25 system-ui;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${safe(text)}</small>`).join('');
    const labelHeight = hasText ? 34 + textLines.length * 15 : 34;
    return L.divIcon({
      className: 'cctv-unified-marker-host',
      html: `<div style="position:relative;display:flex;align-items:center;flex-direction:column;min-width:34px;filter:drop-shadow(0 2px 3px #17203388)"><span style="display:grid;place-items:center;width:34px;height:34px;border:2px solid #fff;border-radius:50% 50% 50% 0;background:${safe(item.color)};color:#fff;font:700 16px/1 system-ui;transform:rotate(-45deg)"><b style="transform:rotate(45deg);font:inherit">${symbolHtml(item.symbol)}</b></span>${hasText ? `<span style="margin-top:2px;border-radius:5px;overflow:hidden">${labelHtml}</span>` : ''}</div>`,
      iconSize: [hasText ? 152 : 34, hasText ? labelHeight : 38],
      iconAnchor: [hasText ? 76 : 17, hasText ? labelHeight - 1 : 34],
      popupAnchor: [0, hasText ? -labelHeight : -34]
    });
  };
