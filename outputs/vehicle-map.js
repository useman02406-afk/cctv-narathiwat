  ...
  async function init(){
    ...
    try{
      ...
      function render(){
        ...
        cluster.clearLayers(); markers=[]; const markersByIndex=new Map(), markerByRow=new Map(); filtered.forEach(row=>{if(!validPoint(row)) return; const marker=L.marker([latitude(row),longitude(row)]).bindPopup(popup(row),{maxWidth:340}); markers.push(marker); markerByRow.set(row,marker); cluster.addLayer(marker);}); slice.forEach((row,index)=>{const marker=markerByRow.get(row);if(marker)markersByIndex.set(index,marker);});
